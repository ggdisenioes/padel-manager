...
export default function PlayerProfile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (pData) {
        ...
        // 2. Buscar todos sus partidos (donde aparezca su nombre en cualquier posición)
        // Nota: Usamos su nombre porque así lo guardamos en la tabla matches por simplicidad
  ...
  const calculateStats = (matchList: any[], playerName: string) => {
      ...
      matchList.forEach(m => {
          if (m.winner === 'pending') return; // Ignoramos los no jugados
          played++;
          
          // Averiguar en qué equipo estaba el jugador (A o B)
          const isTeamA = m.player_1_a === playerName || m.player_1_b === playerName;
          
          // Si estaba en A y ganó A, o estaba en B y ganó B -> Victoria
